...
Единый скрипт для:
  ...
  Для запуска онлайн теста:
  python main_calib_live_outdet_run.py run --profile home --model_dir lora_tune/models/run_2026-02-25_19-07-15/best_model --base_conf_th 0.6 --debounce_s 1.0 --outlier_detector outlier_detector.pkl
"""
...
# Relative Windows-style back-slash paths have been replaced with
# config-resolved values so this script works on any OS.
...
def run_realtime(model_dir, profile="default", base_conf_th=0.6,
                 debounce_s=1.0, outlier_detector=None,
                 onnx_dir=None, onnx_int8=False,
                 energy_th_override=None):
    from core.realtime_recognizer import RealTimeRecognizer

    labels, conf_th_per_label, energy_th = load_profile_settings(
        model_dir, profile
    )
    if energy_th_override is not None:
        energy_th = energy_th_override
    ...
    rec = RealTimeRecognizer(
        ...
        sd_blocksize=8000,
        onnx_dir=onnx_dir,
        onnx_use_int8=onnx_int8,
    )

    def on_detect(d):
        print(f"  >>> [{profile}] {d['label']} ({d['prob']:.3f})")
...
def offline_mic_test(
    ...
    for i in range(0, len(audio) - win_samples + 1, stride_samples):
        ...
        if is_outlier:
            # print(f"  t={i/sr:5.2f}s: OUTLIER (dist={out_info['min_distance']:.3f} > th={out_info['threshold']:.3f})")
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Калибровка и запуск голосового ассистента (Wav2Vec2 + RealTimeRecognizer + OutlierDetection)"
    )
    ...
    run_p.add_argument(
        ...
        type=str,
        default=None,
        help="Путь к outlier_detector.pkl (если указан — в онлайн-режиме включается outlier detection)"
    )
    run_p.add_argument(
        "--onnx_dir",
        type=str,
        default=None,
        help="Директория с ONNX-моделью (model_fp32.onnx / model_int8.onnx + onnx_config.json). "
             "Если указан — используется ONNX Runtime вместо PyTorch."
    )
    run_p.add_argument(
        "--onnx_int8",
        action="store_true",
        help="Использовать model_int8.onnx вместо model_fp32.onnx (требует --onnx_dir)."
    )
    run_p.add_argument(
        "--energy_th",
        type=float,
        default=None,
        help="Переопределить порог энергии (e.g. 1e-3). По умолчанию берётся из config.json (1e-4)."
    )
    ...
    test_p.add_argument(
        ...
        type=str,
        default=None,
        help="Путь к outlier_detector.pkl (если указан — включается outlier detection)"
    )
    ...
    if args.command == "calibrate":
        ...
        run_realtime(
            ...
            energy_th_override=args.energy_th,
        )
